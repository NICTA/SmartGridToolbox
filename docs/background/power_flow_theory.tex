...
Due to the basic relation $S = I^*V$, the power loss between bus $i$ and $k$ is $S_{ik} = I_{ik}^*(V_i - V_k)$. Note that power is a scalar: if we reverse the order of the busses, then the power loss is unchanged, in contrast to, say, a current, where reversing the order of the busses will negate the current. However, a power injection into a bus can be defined as the bus voltage times the current injection, so in that sense, power can be considered as a proxy for current -- and, indeed, you will usually read about power flows rather than current flows.
...
\begin{align}
	...
	\end{bmatrix}
\end{align}
The real part of $z$ is positive and corresponds to a resistance, and the imaginary part is also positive and corresponds to an inductance. Both the inductance and impedance are proportional to the length of the line.

Longer transmission lines ($> 80$ km) also develop a capacitance between the line and ground. This is usually modelled using the $\pi$-model. 
...
\begin{align}
I_{AB} &= \frac{y_l}{|n|^2}(V_A - V_B) - \frac{y_l}{a^*}V_a \\
I_{BC} &= \frac{y_l}{|n|^2}(V_B - V_C) - \frac{y_l}{a^*}V_b \\
I_{CA} &= \frac{y_l}{|n|^2}(V_C - V_A) - \frac{y_l}{a^*}V_c \\
...
\begin{align}
I_A &= I_{AB} - I_{CA} \\
&= \frac{y_l}{|n|^2}(2V_A - V_B - V_C) + \frac{y_l}{a^*}(V_c - V_a) \\
I_B &= I_{BC} - I_{AB} \\
&= \frac{y_l}{|n|^2}(2V_B - V_C - V_A) + \frac{y_l}{a^*}(V_a - V_b) \\
I_C &= I_{CA} - I_{BC} \\
&= \frac{y_l}{|n|^2}(2V_C - V_A - V_B) + \frac{y_l}{a^*}(V_b - V_c)
\end{align}
So we can immediately write down the nodal admittance relationship:
\begin{align}
\begin{bmatrix}I_A \\ I_B \\ I_C \\ I_a \\ I_b \\ I_c\end{bmatrix} &=
y_l \begin{bmatrix}
	2/|n|^2 & -1/|n|^2 & -1/|n|^2 & -1/a^* & 0 & 1/a^* \\
	-1/|n|^2 & 2/|n|^2 &  -1/|n|^2 & 1/a^*  & -1/a^* & 0 \\
...
\begin{figure}[!h]
	\begin{center}
		\includegraphics[width=(\textwidth-2cm)]{branch.png}
	\end{center}
	\caption{
...
\begin{align}
	...
	\begin{bmatrix}V_0 \\ V_1\end{bmatrix}
\end{align}
Normally, $y_b = y_c$ for both transformers and transmission lines. For a transmission line, $y_b = y_c = jb_c/2$, where $b_c$ is the branch charging capacitance, as defined in the {\sc Matpower} manual. For a transformer, $b_c$ is negative, i.e. it is an inductance rather than a capacitance. For both transformers and lines, $y_a$ has a real positive part (corresponding to the resistance) and a negative imaginary part (an inductance).
