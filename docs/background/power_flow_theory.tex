...
\begin{align}
	...
	&- \M{i}\sm{\M{k}\left(G_{ik}\cs{ik} + B_{ik}\sn{ik}\right)} \notag \\
	Q_i &= Q_{ci} + Q_{gi} - \Ii_{ci}M_i + b_i\M{i}^2 \notag \\
	&- \M{i}\sm{\M{k}\left(G_{ik}\sn{ik} - B_{ik}\cs{ik}\right)} \notag \\
\end{align}
...
\begin{align}
	...
	\frac{\partial P_i}{\partial \theta_k} &= -\M{i}\M{k}(1 - \delta_{ik})(G_{ik}\sn{ik} - B_{ik}\cs{ik}) \notag \\
	...
	\frac{\partial Q_i}{\partial \theta_k} &= \M{i}\M{k}(1 - \delta_{ik})(G_{ik}\cs{ik} + B_{ik}\sn{ik})
\end{align}

This gives us enough information to carry out the NR method for polar coordinates.

\subsection{Fast-decoupled load flow}
Especially in transmission networks, we have that $G_{ik} << B_{ik}$ and $\theta_{ik} \approx 0$. Under these conditions, the Jacobian above can be simplified:
\begin{align}
	\frac{\partial P_i}{\partial \M{k}} &= \left(\Ir_{ci} - 2g_i\M{i}\right)\delta_{ik}, & k \in \text{PQ} \notag \\
	\frac{\partial P_i}{\partial Q_{gk}} &= 0, &k \in \text{PV}\notag \\
	\frac{\partial P_i}{\partial \theta_k} &= \M{i}\M{k}(1 - \delta_{ik})B_{ik} \notag \\
	\frac{\partial Q_i}{\partial \M{k}} &= \left(-\Ii_{ci} + 2b_i\M{i}\right)\delta_{ik} + \M{i}(1 + \delta_{ik})B_{ik}, & k \in \text{PQ} \notag \\
	\frac{\partial Q_i}{\partial Q_{gk}} &= \delta_{ik}, & k \in \text{PV} \notag \\
