...
\begin{align}
	...
	&- \M{i}\sum_{k = 0}^N{\left(G_{ik}\cs{ik} + B_{ik}\sn{ik}\right)\M{k}} \notag \\
	Q_i &= Q_{ci} + Q_{gi} - \Ii_{ci}M_i + b_i\M{i}^2 \notag \\
	&- \M{i}\sum_{k = 0}^N{\left(G_{ik}\sn{ik} - B_{ik}\cs{ik}\right)\M{k}} \notag \\
\end{align}
...
\begin{align}
	...
	\frac{\partial Q_i}{\partial \M{k}} &= 
		\begin{cases}
			...
			-\Ii_{ci} + 2b_i\M{i} - \sum_{l = 0}^N{(1 + \delta_{il})\left(G_{il}\sn{il} - B_{il}\cs{il}\right)M_l}, & k = i, \\
...
\begin{align}
	...
	\frac{\partial Q_i}{\partial \M{k}} &= 
		\begin{cases}
			\M{i}B_{ik}, & k \ne i \\
			-\Ii_{ci} + 2b_i\M{i} + \sum_{l = 0}^N{(1 + \delta_{il})B_{il}M_l}, & k = i, \\
		\end{cases} \notag \\
	\frac{\partial Q_i}{\partial Q_{gk}} &= \delta_{ik}, & k \in \text{PV} \notag \\
