...
For PV busses, the power flow equations also hold, but with $Q_{\text{bus}}$ being considered as a variable rather than a constant, and an extra constraint:
\begin{align}
...
\end{align}
then the constraint will always continue be satisfied. Thus, $\re{\Delta V}$ may be eliminated from the NR equations. (This result may easily be more formally derived by considering the Jacobian terms in the constraint). Any terms that would normally be in the $\re{\Delta V}_i$ column of the Jacobian will then need to be multiplied by $-\im{V}_i/\re{V}_i$ and then shifted to the $\re{\Delta V}_i$ column.
