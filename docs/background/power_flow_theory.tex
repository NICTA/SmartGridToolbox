...
In balanced positive sequence three-phase systems, the phase to neutral voltages at a bus, or phase voltages, are $V_A = V$, $V_B = V\exp(-2j\pi/3)$ and $V_C = V\exp(2j\pi/3)$. The line to line voltages, also just called the line voltages, are $V_{AB}$, $V_{BC}$, $V_{CA}$, where, for example, $V_{AB} = V_A - V_B = \sqrt{3}V_A\exp(j\pi/6)$. Similarly, $V_{BC} = \sqrt{3}V_A\exp(-j\pi/2)$ and $V_{CA} = \sqrt{3}V_A\exp(5j\pi/6)$ Thus, the magnitude of the line to line voltages is $\sqrt{3}$ times the magnitude of the phase voltages.
...
Balanced power flow can be analysed in terms of a single phase of the circuit. The power will be three times the power in that phase. For a Y-connected load,
\begin{align}
S &= 3I_{A}^*V_{A} \notag \\
&= \sqrt{3}I_{A}^*V_{AB}\exp(-j\pi/6)
\end{align}
and for a Y-connected load,
\begin{align}
S &= 3I_{AB}^*V_{AB} \notag \\
&= \sqrt{3}I_{A}^*V_{AB}\exp(-j\pi/6)
\end{align}
Thus, regardless of whether we have a $\Delta$ or a $Y$-connected load, the power is often expressed as
\begin{align}
S &= \sqrt{3}I_{l}^*V_{l}.
\end{align}
where $I_{l}$ is the line current, $V_l$ is the line to line voltage, and we have absorbed a constant phase factor. 
...
\begin{align}
	...
	\right]
\end{align}
where $\alpha := \exp(2\pi j/3)$, and it's inverse is then
\begin{align}
	A^{-1} &= \frac{1}{3}\left[
		\begin{array}{lll}
			1 & 1 & 1 \\
...
Suppose we have a balanced positive sequence voltage $V^+_{ABC} = V_0[1, \alpha^2, \alpha]^T$. Then, under the transformation to symmetric components, we find $V_{012} = V_0[0, 1, 0]^T$. Similarly, a negative sequence voltage $V^-_{ABC} = V_0[1, \alpha, \alpha^2]^T$ transforms to $V_{012} = V_0[0, 0, 1]^T$. Finally, the unbalanced \emph{zero sequence} voltage $V^-_{ABC} = V_0[1, 1, 1]^T$ transforms to $V_{012} = V_0[1, 0, 0]^T$. Therefore, the transformation $A^{-1}$ decomposes $V_{ABC}$ into a positive sequence component $V_1$, a negative sequence component $V_2$, and a zero sequence component $V_0$. For positive sequence balanced power flow, only $V_1$ will be non-zero.

Now consider a three phase transmission line. The equation for the voltage drop in each wire is:
\begin{align}
	\Delta V = ZI
\end{align}
where $Z$ is the phase impedance matrix for the line. If the power flow is to be perfectly balanced, then there must be a symmetry under exchange of any two phases, meaning all wires must be located on an equilateral triangle and must have the same impedance. The $Z$ must have the form:
\begin{align}
	Z_{ABC} = \left[
			\begin{array}{lll}
				z & m & m \\
...
\begin{align}
	Z_{012} = \left[
		...
		\label{EQ_Z012}
\end{align}
This matrix is diagonal, and therefore a perfectly symmetric network can be decomposed under the transformation to symmetric components into three independent networks: a positive sequence network, a negative sequence network, and a zero sequence network.
...
\begin{align}
	V &\leftarrow V_1, \text{the voltage in phase A} \notag \\
	I &\leftarrow I_1, \text{the line current along phase A wire} \notag \\
	Z &\leftarrow Z_{11}, \text{line impedance, equal to $z - m$, see Eq. \ref{EQ_Z012}.}
\end{align}

What about power? While we could just
