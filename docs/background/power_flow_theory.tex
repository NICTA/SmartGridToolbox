...
In balanced positive sequence three-phase systems, the phase to neutral voltages (or, simply, the phase voltages), at a bus, are $V_A = V$, $V_B = V\exp(-2j\pi/3)$ and $V_C = V\exp(2j\pi/3)$. The line to line voltages, also just called the line voltages, are $V_{AB} = V_A - V_B = \sqrt{3}V_A\exp(j\pi/6)$, $V_{BC} = \sqrt{3}V_A\exp(-j\pi/2)$ and $V_{CA} = \sqrt{3}V_A\exp(5j\pi/6)$ Thus, the magnitude of the line to line voltages is $\sqrt{3}$ times the magnitude of the phase voltages.
...
The currents along the phase wires, $I_A, I_B, I_C$ are known as the line currents. Note that this can be confusing when compared to the common use of the term ``line voltage'', meaning line to line voltage. For a $\Delta$-connected load, we can easily show, using Ohm's law, that $I_{BC} = \exp(-2j\pi/3) I_{AB}$ and $I_{CA} = \exp(2j\pi/3) I_{AB}$. Using Kirchoff's current law, we then have,
\begin{align}
I_{A} &= I_{AB} - I_{CA} \notag \\
&= I_{AB}(1 - \exp(2j\pi/3)) \notag \\
\rightarrow I_{AB} &= \frac{1}{\sqrt{3}}\exp\left(j\pi/6\right)I_A
\end{align}
Meaning the $\Delta$ currents are a factor of $\sqrt{3}$ less than the line currents, which is opposite to the relation that holds for the voltages.

Note that for every $\Delta$ connected load, there is an equivalent $Y$-connected load.
...
\begin{align}
	...
	\right]
\end{align}
where $\alpha := \exp(2\pi j/3)$. We then have,
\begin{align}
	A^{-1} &= \frac{1}{3}\left[
		\begin{array}{lll}
			1 & 1 & 1 \\
...
Suppose we have a balanced positive sequence voltage $V^+_{ABC} = V_0[1, \alpha^2, \alpha]^T$. Then, under the transformation to symmetric components, we find $V_{012} = V_0[0, 1, 0]^T$. Similarly, a negative sequence voltage $V^-_{ABC} = V_0[1, \alpha, \alpha^2]^T$ transforms to $V_{012} = V_0[0, 0, 1]^T$. Finally, the unbalanced \emph{zero sequence} voltage $V^-_{ABC} = V_0[1, 1, 1]^T$ transforms to $V_{012} = V_0[1, 0, 0]^T$. Therefore, the transformation $A^{-1}$ decomposes $V_{ABC}$ into a positive sequence component $V_1$, a negative sequence component $V_2$, and a zero sequence component $V_0$.

\subsection{Reduction of a three-phase balanced problem to single phase}

In balanced power flow, there is only a positive sequence component, which suggests that symmetric components could be used to reduce a three-phase problem to an equivalent single-phase problem. However, using the treatment above, it turns out that we then need to remember to artificially multiply by three when calculating the total power. This is not ideal, because total power is an important physical quantity that corresponds to the amount of actual energy consumed or generated. Therefore, we shall develop a non-standard treatment of symmetric components.
...
\begin{align}
	...
	\right]
\end{align}
Note that $B = \sqrt(3)A^{-1}$, and also that $B^{-1} = B^\dag$, so $B$ is a unitary matrix.
...
\end{align}
So for example, a positive sequence balanced three phase voltage will have a single nonzero positive sequence component $V_1$, effectively equal to the line to line voltage (or, alternatively whose magnitude is the RMS of the phase voltages). A similar relationship holds for the line currents, but this time there is no simple interpretation of $\sqrt{3}I_A$ analogous to $\sqrt{3}V_A$ being interpreted as the line to line voltage (although it's magnitude is still the RMS line current).
...
For balanced positive sequence power flow, we need only be concerned with the positive sequence network, so the problem becomes scalar, in other words the three phase problem reduces to an equivalent single phase problem. Quantities are interpreted as follows:
\begin{align}
	V &\leftarrow V_1,\;\text{$\sqrt{3} \times$ phase A voltage (effectively the line to line voltage, or RMS),} \notag \\
	I &\leftarrow I_1,\;\text{$\sqrt{3} \times$ the line current in phase A, effectively the RMS,} \notag \\
	Z &\leftarrow Z_{11},\;\text{line impedance, equal to $z - m$, see Eq. \ref{EQ_Z012}.}
\end{align}

What about power? For a positive sequence balanced system, we have, in terms of sequence components,
\begin{align}
	S = I^\dag V = 3I_A^*V_A
\end{align}
which is indeed the total power.
...
\begin{itemize}
	\item The voltage in the equivalent single phase system is the line to line voltage in the three phase system (but, to be pedantic, with the same phase as A). To get the phase A voltage, divide the single phase voltage by $\sqrt{3}$.
	\item To get the current along each phase of a three phase line, divide the single phase current by $\sqrt{3}$. This is most important when calculating line limits.
	\item Power in the single phase system is the \emph{total} power of all phases in the three phase system. Thus, for example, the power at a $PQ$ bus for the single phase system is the sum of the power for all phases in the three phase system. This is regardless of whether the load or source is $\Delta$ or $Y$-connected.
