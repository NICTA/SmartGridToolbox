...
\begin{figure}[!h]
	\begin{center}
		\includegraphics[width=(8cm)]{DeltaY}
	\end{center}
	\caption{
		Delta and Y circuits for balanced three phase power.
	}
	\label{FIG_PI_LINE}
\end{figure}
In balanced positive sequence three-phase systems, the voltages are $V_A = V$, $V_B = V\exp(-2\pi/3)$ and $V_C = V\exp(2\pi / 3)$, and the currents have the same phase relationship. For balanced loads, we can model the system by considering only one of the phases (e.g. A, or A-B), knowing that all the currents and voltages will be duplicated for the other two with the appropriate phase shifts. 

Delta-connected loads are mathematically equivalent to Y-connected loads, with the following relationships holding if the total power is the same in both cases
\begin{align}
	V_{AB} &= \sqrt{3}\exp\left(j\frac{\pi}{6}\right)V_A \notag \\
	V_{BC} &= \sqrt{3}\exp\left(j\frac{\pi}{6}\right)V_B \notag \\
	V_{CA} &= \sqrt{3}\exp\left(j\frac{\pi}{6}\right)V_C \notag \\
	I_{AB} &= \frac{1}{\sqrt{3}}\exp\left(j\frac{\pi}{6}\right)I_A \notag \\
	I_{BC} &= \frac{1}{\sqrt{3}}\exp\left(j\frac{\pi}{6}\right)I_B \notag \\
	I_{CA} &= \frac{1}{\sqrt{3}}\exp\left(j\frac{\pi}{6}\right)I_C \notag \\
	Z_\Delta &= 3 Z_Y
\end{align}
...
\section{Carson's Equations: Derivation)
We first deal with a single wire, earth return circuit.
\end{document}