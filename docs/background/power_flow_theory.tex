...
We have
\begin{align}
	\frac{\left|V_\text{out}\right|^2}{\left|V_\text{in}\right|^2} &= \frac{\left|V_\text{in} - IZ\right|^2}{\left|V_\text{in}\right|^2} \notag \\
	&= \frac{\left|V_\text{in} - \frac{S^*_\text{in}Z}{V^*_\text{in}}\right|^2}{\left|V_\text{in}\right|^2} \notag \\
	&= 1 - \frac{2\re{S^*_\text{in}Z}}{\left|V_\text{in}\right|^2} + \frac{\left|S\right|^2\left|Z\right|^2}{\left|V_\text{in}\right|^4} \notag \\
	&= 1 - \frac{2(PR + QX)}{\left|V_\text{in}\right|^2} + \frac{\left|S\right|^2\left|Z\right|^2}{\left|V_\text{in}\right|^4} \notag
\end{align}
At high voltages, the middle term dominates the voltage drop. High voltage lines normally also have a larger reactance than resistance, so reactive power is a more significant contributor to the voltage drop than is real power.
