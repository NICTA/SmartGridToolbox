...
A real transformer includes a leakage impedance (due to finite resistance of copper windings and core losses) and a shunt magnetising impedance. There are various ways of drawing an equivalent circuit; the one used by matpower is shown in Fig. \ref{FIG_MATPOWER_BRANCH_MODEL}.
\begin{figure}[!h]
	\begin{center}
		\includegraphics[width=(\textwidth-2cm)]{fig_matpower_branch_model}
	\end{center}
	\caption{
		The branch model used by matpower: removing the transformer (setting $N$ to 1) results in a $\pi$ model of a transmission line, while including the transformer results in a model for a real transformer, with $y_s$ being the leakage impedance and $jb_c$ being the magnetising impedance.
	}
	\label{FIG_MATPOWER_BRANCH_MODEL}
\end{figure}
