...
A synchronous generator has a flywheel with moment of inertia $J$. There is a mechanical torque (e.g. from the turbine) $T_m$ pushing the rotor and an electrical torque $T_e$ that comes from the alternator and usually acts against the mechanical torque. We define the torque imbalance $\Delta T = T_m - T_e$. When $\Delta T$ is positive the rotor will speed up and when it is negative it will slow down. We define the rotor angle $\theta$ which is assumed to be zero at $t = 0$. For a rotor running at the mandated synchronous angular frequency $\omega_s$, we then have $\theta = \omega_s t$. Because the rotor may deviate from this frequency, we define the angular error
\begin{align}
	\delta &= \theta - \omega_s t. \label{EQ_DEFN_DELTA}
\end{align}
It is this quantity that we wish to calculate.

The equation governing the motion of the rotor is
\begin{align}
	J \frac{\partial^2 \theta}{\partial t^2} &= \Delta T
\end{align}
which is analogous to Newton's equation $F = ma$. Using Eq. \ref{EQ_DEFN_DELTA}, we then have that
\begin{align}
	J \frac{\partial^2 \delta}{\partial t^2} = \Delta T
\end{align}
Since the power $P = \omega T$ where $\omega = \partial \theta / \partial t$, we multiply both sides by $\omega$ to obtain a relation for the power mismatch:
\begin{align}
	J\omega \frac{\partial^2 \delta}{\partial t^2} &= \Delta P \notag \\
	J\left(\omega_s + \frac{\partial \delta}{\partial t}\right) \frac{\partial^2 \delta}{\partial t^2} &= \Delta P
\end{align}
where $\Delta P$ is the input mechanical power minus the electrical power supplied to the network. When the rotor is running far from the mandated frequency, then this equation would need to be applied as is. However, at frequencies near the network frequency, $\omega_s \gg \partial \delta / \partial t$, and so we can write
\begin{align}
	J\omega_s \frac{\partial^2 \delta}{\partial t^2} &= \Delta P \notag \\
	L_s \frac{\partial^2 \delta}{\partial t^2} &= \Delta P \label{EQ_SWING}
\end{align}
where $L_s = J \omega_s$ is the angular momentum of the rotor at the network frequency. This is the swing equation.
...
Note that for the purposes of the swing equations, we are only interested in the real power component of the power flow equations.
