...
\end{center}
\end{figure}
We use the open delta model shown in \ref{FIG_OPEN_DELTA}. Using the equations for an ideal transformer, Eq. (\ref{EQ_REAL_TRANS}) (and neglecting magnetising impedance),  we can derive the following equations:
\begin{align}
...
\end{align}
We assume that both sides of of the transformer are ungrounded, and therefore there is no zero sequence current: $I_A + I_B + IC = I_a + I_b + I_c = 0$. Thus, $I_B = -I_A - I_C$, and $I_b = -I_a - I_c$. Since $I = YV$, we can deduce the bus admittance matrix $Y$ as:
\begin{align}
Y &= 
\begin{bmatrix}
	\frac{y_{LA}}{|n_A|^2} & -\frac{y_{LA}}{|n_A|^2} & 0 & -\frac{y_{LA}}{n_A^*} & \frac{y_{LA}}{n_A^*} & 0 \\
	%
	-\frac{y_{LA}}{|n_A|^2} & \left(\frac{y_{LA}}{|n_A|^2} + \frac{y_{LC}}{|n_C|^2}\right) & -\frac{y_{LC}}{|n_C|^2} & \frac{y_{LA}}{n_A^*} & \left(-\frac{y_{LA}}{n_A^*} - \frac{y_{LC}}{n_C^*}\right) & \frac{y_{LC}}{n_C^*} \\
	%
	0 & -\frac{y_{LC}}{|n_C|^2} & \frac{y_{LC}}{|n_C|^2} & 0 & \frac{y_{LC}}{n_C^*} &  -\frac{y_{LC}}{n_C^*} \\
	%
	-\frac{y_{LA}}{n_A} & \frac{y_{LA}}{n_A} & 0 & y_{LA} & -y_{LA} & 0 \\
	%
	\frac{y_{LA}}{n_A} & \left(-\frac{y_{LA}}{n_A} - \frac{y_{LC}}{n_C}\right) & \frac{y_{LC}}{n_C} & -y_{LA} & \left(y_{LA} + y_{LC}\right) & -y_{LC} \\
	%
	0 & \frac{y_{LC}}{n_C} & -\frac{y_{LC}}{n_C} & 0 & -y_{LC}\ & y_{LC}
\end{bmatrix}
\end{align}
However, there is an issue here, in that the neutral point of the secondary floats, which may cause numerical issues when solving the Newton Raphson equations. We solve this issue by noting that $V_B = V_b$, which may be implemented by making changes to the second and fifth rows of this equation, as follows:
\begin{align}
Y &= 
\begin{bmatrix}
	\frac{y_{LA}}{|n_A|^2} & -\frac{y_{LA}}{|n_A|^2} & 0 & -\frac{y_{LA}}{n_A^*} & \frac{y_{LA}}{n_A^*} & 0 \\
	%
	-\frac{y_{LA}}{|n_A|^2} & \left(\frac{y_{LA}}{|n_A|^2} + \frac{y_{LC}}{|n_C|^2} - \frac{y_{LA}}{n_A^*} - \frac{y_{LC}}{n_C^*}\right) & -\frac{y_{LC}}{|n_C|^2} & \frac{y_{LA}}{n_A^*} & 0 & \frac{y_{LC}}{n_C^*} \\
	%
	0 & -\frac{y_{LC}}{|n_C|^2} & \frac{y_{LC}}{|n_C|^2} & 0 & \frac{y_{LC}}{n_C^*} &  -\frac{y_{LC}}{n_C^*} \\
	%
	-\frac{y_{LA}}{n_A} & \frac{y_{LA}}{n_A} & 0 & y_{LA} & -y_{LA} & 0 \\
	%
	\frac{y_{LA}}{n_A} & 0 & \frac{y_{LC}}{n_C} & -y_{LA} & \left(y_{LA} + y_{LC}-\frac{y_{LA}}{n_A} - \frac{y_{LC}}{n_C}\right) & -y_{LC} \\
	%
	0 & \frac{y_{LC}}{n_C} & -\frac{y_{LC}}{n_C} & 0 & -y_{LC}\ & y_{LC}
\end{bmatrix}
\end{align}
