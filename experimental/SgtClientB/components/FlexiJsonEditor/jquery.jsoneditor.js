...
(function( $ ) {
    ...
    $.fn.jsonEditor = function(json, options) {
        options = options || {};
        stringify.replacer = options.replacer;
        ...
        return this.each(function() {
            JSONEditor($(this), json, options.keys, onchange, onpropertyclick, options.propertyElement, options.valueElement);
        });
        
    };
    
    function JSONEditor(target, json, keys, onchange, onpropertyclick, propertyElement, valueElement) {
        var opt = {
            target: target,
            onchange: onchange,
        ...
        construct(opt, json, keys, opt.target);
    ...
    function stringify(obj) {
        var res;
        try {
            if (stringify.replacer != null) {
                    res = JSON.stringify(obj, stringify.replacer);
                } else {
    ...
    function construct(opt, json, keys, root, path) {
        if (keys == null) {
            keys = [];
            for (var key in json) {
        ...
        for (var i = 0; i < keys.length; ++i) {
            ...
            if (isObject(json[key]) || isArray(json[key])) {
                construct(opt, json[key], null, item, (path ? path + '.' : '') + key);
            }
        }
    }
    ...
    function valueChanged(opt) {
        return function() {
            ...
            if ((isObject(val) || isArray(val)) && !$.isEmptyObject(val)) {
                construct(opt, val, null, item, (path ? path + '.' : '') + key);
                addExpander(item);
            } else {
