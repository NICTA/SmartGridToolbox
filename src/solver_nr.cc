...
namespace SmartGridToolbox
{
                   ...
                   bool *bad_computations)
   {
      ...
      *bad_computations = false;
      ...
      {
         // Build the diagnoal elements of the bus admittance matrix - this
         // should only happen once no matter what.
         if (BA_diag == NULL)
         {
         ...
         for (indexer=0; indexer<bus_count; indexer++) 
         {
            ...
            {
               // Assign jindexer as intermediate variable (easier for me this
               // way)
                   ...
                   (branch[jindexer].to == indexer))
               {
                  ...
                  {
                     // Split phase - add in 2x2 element to upper left 2x2
                     if (branch[jindexer].from == indexer) // From branch
                     {
                        // End of SPCT transformer requires slightly different
                        // Diagonal components (so when it's the To bus of
                        // SPCT and from for other triplex
