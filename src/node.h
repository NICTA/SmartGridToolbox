...
#define shuntA shunt[0]                         ///< phase A shunt admittance accumulator 
                                                /** (reset to 1/impedance on each pass) */
#define shuntB shunt[1]                         ///< phase B shunt admittance accumulator
                                                /** (reset to 1/impedance on each pass) */
#define shuntC shunt[2]                         ///< phase C shunt admittance accumulator
                                                /** (reset to 1/impedance on each pass) */
...
typedef enum {
        ...
        DIFF_CHILD=4,                           ///< defines child node, has different phase-connection than parent
        DIFF_PARENT=5                           ///< defines a parent, but has a different phase-connection than child
        } SUBNODETYPE;
...
private:
    ...
    complex last_child_current12;               ///< Previous current value 
                                                /** used for child object propogation (namely triplex) */
public:
    ...
    complex *prev_voltage_value;                ///< Pointer for array used to store prev V value for Master/Slave
    complex *prev_power_value;                  ///< Pointer for array used to store prev P value for Master/Slave
public:
    // status
    enum {
    ...
    complex nom_res_curr[3];                    ///< Used for the inclusion of nominal residential currents 
                                                /** (for angle adjustments) */
    bool house_present;                         ///< Indicator flag for a house being attached (NR primarily)
    complex *Triplex_Data;                      ///< Link to triplex line for extra current calc information (NR)
    ...
    int *NR_subnode_reference;                  ///< Pointer to parent node's reference in NR_busdata
...
public:
    ...
    object TopologicalParent;                   ///< Child node's original parent as per the topological config ing GLM
