...
public: /// @todo make this private and create interfaces to control values
    ...
    complex *YSfrom;                   ///< Pointer to 3x3 matrix representing admittance seen from "from" side (transformers)
    complex *YSto;                     ///< Pointer to 3x3 matrix representing admittance seen from "to" side (transformers)
    ...
    void calc_currents(complex *Current_Vals);  ///< Perform "immediate" current calculation - for restoration object
    int link_fault_on(OBJECT **protect_obj, char *fault_type, int *implemented_fault, TIMESTAMP *repair_time,
          void *Extra_Data);           ///< Function to create fault on line
    int link_fault_off(int *implemented_fault, char *imp_fault_name, void *Extra_Data);
                                       ///< Function to remove fault from line
    double mean_repair_time;
    double *link_limits[2];            ///< pointers for line limits (emergency vs. continuous) for link objects.
...
#define CONTROL_FAILED 4               ///< use this status to indicate a controller failure (e.g., PLC failure)
    ...
    set get_flow(class node **from, class node **to) const; ///< determine flow direction
                                                            /**< (return phases on which flow is reverse) */
    ...
    void fault_current_calc(complex C[7][7], unsigned int removed_phase, double fault_type);
    // function traces up from fault to swing bus summing up the link objects' impedances, then calculates the
    // fault current then passes that value back down to the faulted link objects.
